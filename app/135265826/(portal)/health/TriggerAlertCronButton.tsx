...
export default function TriggerAlertCronButton() {
  ...
  async function run() {
    ...
    setDetail('');
    try {
      const res = await fetch('/api/admin/run-send-alerts', { method: 'POST' });
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        setDetail(`${data.sent ?? 0} email${data.sent !== 1 ? 's' : ''} sent`);
        setState('done');
