...
export async function GET(req: NextRequest) {
  try {
    ...
    return await cronRun('Daily Alert Cron', async (record) => {
      const sb = supabase();
      ...
      if (!searches?.length) {
        record('No verified saved searches — nothing to send');
      ...
      record(`Sent ${sent} alert email${sent !== 1 ? 's' : ''} to subscribers`);
      return NextResponse.json({ sent });
    });
  } catch (_err) {
    const _msg = _err instanceof Error ? _err.message : String(_err);
    const _stack = _err instanceof Error ? _err.stack : undefined;
    await logError({ source: 'Cron › Send-alerts', message: _msg, details: _stack, endpoint: 'GET /api/cron/send-alerts', http_status: 500 });
