...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await logError({ source: 'Listings › :id › Applications', message: _msg, details: _stack, endpoint: 'GET /api/listings/[id]/applications', http_status: 500 });
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const _msg = _err instanceof Error ? _err.message : String(_err);
    const _stack = _err instanceof Error ? _err.stack : undefined;
    await logError({ source: 'Listings › :id › Applications', message: _msg, details: _stack, endpoint: 'PATCH /api/listings/[id]/applications', http_status: 500 });
    return NextResponse.json({ error: _msg }, { status: 500 });
  }
}
