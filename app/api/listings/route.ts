...
export async function GET(req: NextRequest) {
  ...
  const mapped = (data ?? []).map((row: Record<string, unknown>) => ({
    ...
    fireplace: row.fireplace ?? false,
    fireplace_location: row.fireplace_location ?? null,
    parking: row.parking ?? false,
    parking_spaces: row.parking_spaces ?? null,
    parking_type: row.parking_type ?? null,
    air_conditioning: row.air_conditioning ?? false,
    heating_type: row.heating_type ?? null,
    pets_policy: row.pets_policy ?? null,
    yard: row.yard ?? false,
    yard_type: row.yard_type ?? null,
...
export async function POST(req: NextRequest) {
  ...
  if (!formData.get('title')?.toString().trim()) validationErrors.push('Title is required.')
  if ((formData.get('description')?.toString() ?? '').trim().length < 30) validationErrors.push('Description must be at least 30 characters.')
  if (!price || price <= 0) validationErrors.push('A valid monthly rent is required.')
  if (bedrooms == null || isNaN(bedrooms) || bedrooms < 0) validationErrors.push('Number of bedrooms is required.')
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      fireplace: formData.get('fireplace') === 'true',
      fireplace_location: formData.get('fireplaceLocation') as string || null,
      parking: formData.get('parking') === 'true',
      parking_spaces: formData.get('parkingSpaces') ? Number(formData.get('parkingSpaces')) : null,
      parking_type: formData.get('parkingType') as string || null,
      air_conditioning: formData.get('airConditioning') === 'true',
      heating_type: formData.get('heatingType') as string || null,
      pets_policy: formData.get('petsPolicy') as string || null,
      yard: formData.get('yard') === 'true',
      yard_type: formData.get('yardType') as string || null,
